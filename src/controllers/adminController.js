...
const uploadMediaToCloud = async (req, res) => {
	console.log("request to upload files to spaces----", req.body);
	try {
		if (
			req.files &&
			req.files["thumbnail"] &&
			...
			const thumbnailFile = req.files["thumbnail"][0];
			const mediaFile = req.files["mediaFile"][0];
			const [playlistId, playlistName] = req.body.playlist.split(",");

			const thumbnail = req.files?.thumbnail
				? await s3service(
						thumbnailFile,
						`thumbnails/${playlistName.replace(/\s+/g, "")}`
				  )
				: null;

			let media = req.files?.mediaFile
				? await s3service(
						mediaFile,
						`media/${playlistName.replace(/\s+/g, "")}`
				  )
				: null;
